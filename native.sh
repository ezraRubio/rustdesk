#!/bin/bash
#
# RustDesk Android Native Library Build Script
# Based on .github/workflows/flutter-build.yml (build-rustdesk-android job)
#
# This script builds only the native library (librustdesk.so) for Android,
# stopping after the .so is compiled and copied to jniLibs. No APK is built.
#
...
print_usage() {
  ...
  echo ""
  echo "Builds only the native library (librustdesk.so) for Android."
  echo ""
  ...
  echo ""
  echo "Output:"
  echo "  flutter/android/app/src/main/jniLibs/<ABI>/librustdesk.so"
}

main() {
  echo "================================================"
  echo "Android Native Library Build Script"
  echo "================================================"
  ...
  JNILIBS_DIR="flutter/android/app/src/main/jniLibs/$ANDROID_ABI"
  echo ""
  info "Native library built successfully!"
  info "Output: $JNILIBS_DIR/librustdesk.so"
  ls -lh "$JNILIBS_DIR/librustdesk.so"
